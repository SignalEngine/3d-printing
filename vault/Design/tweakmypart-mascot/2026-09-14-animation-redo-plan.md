...
- **Mock built instead (James: "Build the mock now, no new clips"):** on the viewer page. Idle clip → CSS zoom into the chest (0.9 s) → crossfade to the close-up keyframe (`key-tablet.png`) → tablet loading screen ("Designing / Checking it prints / Rendering preview") → the trophy model live in three.js on the tablet screen (screen box left 28.5% top 42% w 43.2% h 26.3% of the keyframe) → two CSS dials on the tablet sides that rotate the model and turn with it (drag or arrow keys) → Back zooms out. Verified by a recorded Chromium run (reduce-motion off) and /watch at 2 fps: zoom, tablet, loading, live model and dials all render; no console errors. Two bugs found and fixed in that run: the three.js STLLoader CDN path 404s (replaced with an inline binary STL parser; the earlier tablet mock on this page had the same silent failure) and the loading overlay never hid (a `display:grid` rule beat `[hidden]`).
- **Transform clip (fal topped up, 14 Sep):** 2 takes of `tab-transform` (6 s, ~$1.35). Take 1 used: the chest glass splits into two panels that slide apart, the chamber reconfigures into a flat tablet and small arms/dials come out at its sides while the camera pushes in; it ends aligned with `key-tablet.png` (mean pixel diff 8/255; overlay crisp). Head does not tilt down. Reverse (`tab-transform-rev.mp4`) plays on Back. Recorded Chromium run + /watch at 2 fps: transform → loading → live trophy with dials → reverse back to idle all play. The recorder logged "request failed" on the mp4s; they played in the recording, so these look like aborted range requests to Python's http.server (not confirmed). Knob-twist clip not made: the dials are web controls.
