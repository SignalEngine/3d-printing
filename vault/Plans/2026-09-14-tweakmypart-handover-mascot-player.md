...
Copy from `/root/3d-printing/vault/Design/tweakmypart-mascot/web/` into `public/mascot/`: `idle-a`, `idle-b`, `focus-finish`, `grab-look`, `exit`, `return-wave`, `listening`, `thinking`, `presenting`, `point`, `oops` — each an `.mp4` (632x818, H.264, 24 fps, no audio, black background crushed to true black) plus a `.jpg` poster of its first frame. Commit them (each is under ~1.5 MB).

The story chains end-to-start: `idle-a`/`idle-b` (alternate) → `focus-finish` → `grab-look` → `exit` (ends on empty black) → `return-wave` → idle. The chat clips (`listening`, `thinking`, `presenting`, `point`, `oops`) start and end on the resting pose and play once before returning to idle.
...
1. `components/Mascot.tsx` (client component), a sequence player:
   ...
   - Default: alternate `idle-a` and `idle-b`. Every 3rd loop (make it a prop `storyEvery`, default 3), play the story once: `focus-finish` → `grab-look` → `exit` → `return-wave`, then back to the idles.
   - Prop `mode`: `"idle"` (default, as above) or `"working"` (alternate the idles only, no story; used while a design job runs), plus one-shot props for the chat clips.
   ...
   - `prefers-reduced-motion: reduce` → show the `idle-a.jpg` poster only, no video.
...
3. Tests (vitest, jsdom or happy-dom environment for this file only via a `// @vitest-environment jsdom` comment; add the dev dependency if missing, after the `[ -L node_modules ]` check):
   ...
   - `mode="working"` never leaves the idles;
