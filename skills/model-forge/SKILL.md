...
- **verify_model.py** also WARNs `needs supports` when the part, sliced in its current orientation, has a layer with >50mm² unsupported by the layer below (past a 50° self-supporting overhang), or a mid-air island >2mm² starting with nothing beneath it — this catches a flat horizontal roof over a hollow cavity, which the overhang-angle check misses (the roof's normal points straight up, so it never registers as an "overhang" even though nothing holds it up while printing). WARN, not FAIL: supports are a valid choice. **A `needs supports` WARN means slice with supports (`slice_gate.py --supports tree|normal`) and state the support type in the delivery. Never tell the user "no supports" unless this check is clean.**
- **slice_gate.py** takes `--supports none|tree|normal` (default `none`) to slice with OrcaSlicer supports enabled, and prints whether supports were on plus the filament delta against a no-support baseline slice.
